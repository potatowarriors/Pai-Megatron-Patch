...
- **새 SFT 셋을 추가하기 전에 `examples/alpha/docs/INTERLEAVED_THINKING.md` §"새 SFT 셋 추가 시 규칙 7건"을 읽는다.**
...
- 변환기는 `toolkits/sft_data_preprocessing/build_alpha_sft_idxmap.py`. 마스킹 규약의 정본은
  `examples/alpha/tools/verify_chat_template.py`(31 tests) — 둘이 어긋나면 verify 쪽이 맞다.
