...
#
# Modifications:
# - Added dense_mlp_layer support in MambaStackSubmodules for Dense FFN layers
# - Extended layer building logic to handle DENSE_MLP symbol
...
# Use our extended layer allocation with DENSE_MLP support
from megatron_patch.ssm.mamba_hybrid_layer_allocation import Symbols as LayerSymbols
from megatron_patch.ssm.mamba_hybrid_layer_allocation import allocate_layers
...
class MambaStackSubmodules:
    ...
    Extended to support Dense MLP layers (standard FFN) in addition to MoE MLP layers.

    Attributes:
        mamba_layer: Spec for Mamba (GatedDeltaNet) layers (symbol: M)
        attention_layer: Spec for Attention layers (symbol: *)
        mlp_layer: Spec for MoE MLP layers (symbol: -)
        dense_mlp_layer: Spec for Dense MLP layers (symbol: D) - standard FFN with SwiGLU
    """

    mamba_layer: Union[ModuleSpec, type] = IdentityOp
    attention_layer: Union[ModuleSpec, type] = IdentityOp
    mlp_layer: Union[ModuleSpec, type] = IdentityOp           # MoE MLP (-)
    dense_mlp_layer: Union[ModuleSpec, type] = IdentityOp     # Dense MLP (D)
...
class MambaStack(MegatronModule):
    ...
    Extended to support Dense MLP layers through the 'D' symbol in hybrid_override_pattern.
    ...
    Args:
        ...
        hybrid_override_pattern (str, optional): the hybrid layer pattern to override
             with. Supports: M (Mamba), * (Attention), - (MoE MLP), D (Dense MLP).
             Defaults to None.
    ...
    def __init__(
        ...
        for i, layer_type in enumerate(self.layer_type_list):
            ...
            with fp8_init_context:
                ...
                elif layer_type == LayerSymbols.DENSE_MLP:
                    # Dense MLP layer (standard FFN with SwiGLU)
                    # Transformer layers apply their own pp_layer_offset
                    layer = build_module(
                        submodules.dense_mlp_layer,
                        config=self.config,
                        layer_number=i + 1,
                        pg_collection=pg_collection,
                    )
