...
#
# Modifications:
# - Added DENSE_MLP symbol for Dense FFN layer support (vs MoE MLP)
...
class Symbols:
    ...
    Extended from original Megatron-LM to support Dense MLP layers:
    - M: Mamba (GatedDeltaNet) layer
    - *: Attention layer
    - -: MoE MLP layer (Mixture of Experts)
    ...
    ATTENTION = "*"
    MLP = "-"           # MoE MLP
    DENSE_MLP = "D"     # Dense MLP (NEW)
    VALID = {MAMBA, ATTENTION, MLP, DENSE_MLP}
...
def _layer_counts_match(a: list, b: list) -> bool:
    """Check if layer counts match between two patterns.

    Note: For patterns with Dense MLP (D), we treat D as equivalent to MLP (-)
    for the purpose of ratio matching, since both are MLP-type layers.
    """
    # Normalize: treat D as - for comparison purposes
    def normalize(pattern):
        return [Symbols.MLP if x == Symbols.DENSE_MLP else x for x in pattern]

    a_norm = normalize(a)
    b_norm = normalize(b)

    for s in {Symbols.MAMBA, Symbols.ATTENTION, Symbols.MLP}:
        if a_norm.count(s) != b_norm.count(s):
            return False
    return True
...
def allocate_layers(
    ...
    Supports the following layer types:
    - M: Mamba (GatedDeltaNet)
    - *: Attention
    - -: MoE MLP (Mixture of Experts)
    - D: Dense MLP (standard FFN)

    When using override_pattern with 'D', the Dense MLP layers count towards
    the total MLP ratio (target_mlp_ratio).
    """
    ...
    if override_pattern is not None:
        ...
        if (target_attention_ratio > 0.0 or target_mlp_ratio > 0.0) and not _layer_counts_match(
            ...
            raise ValueError(
                ...
                "pattern. Note: Dense MLP (D) counts as MLP (-) for ratio matching."
            )
    ...
    if target_attention_ratio > 0.0 or target_mlp_ratio > 0.0 or override_pattern is not None:
        ...
        # Count both MoE MLP (-) and Dense MLP (D) as MLP layers
        actual_mlp_layers_count = layer_type_list.count(Symbols.MLP) + layer_type_list.count(Symbols.DENSE_MLP)
        actual_moe_mlp_count = layer_type_list.count(Symbols.MLP)
        actual_dense_mlp_count = layer_type_list.count(Symbols.DENSE_MLP)
        ...
        log_single_rank(
            ...
            f"{Symbols.MLP} is moe_mlp, "
            f"{Symbols.DENSE_MLP} is dense_mlp):",
        )
        ...
        log_single_rank(
            logger,
            logging.INFO,
            f"{actual_mlp_layers_count} mlp layers ({actual_moe_mlp_count} moe + {actual_dense_mlp_count} dense) in "
            f"{total_layers_count} total layers.",
        )
        log_single_rank(
            logger,
...
def get_layer_maps_from_layer_type_list(
    ...
    for each layer type in [Attention, Mamba, MLP, DenseMLP] given a layer type list.

    Extended to support Dense MLP (D) layers.
    """
    layer_types = [Symbols.ATTENTION, Symbols.MAMBA, Symbols.MLP, Symbols.DENSE_MLP]
...
if __name__ == "__main__":
    ...
    test_cases = [
        # Test Dense MLP patterns
        (24, 0.125, 0.5, "MDM-M-*-M-M-M-*-M-M-M-*-"),  # First MLP is Dense
        (24, 0.125, 0.5, "M-M-M-*-M-M-M-*-M-M-M-*-"),  # All MoE (original)
        (9, 0.0, 0.0, "M*-M*-M*-"),
        (9, 0.0, 0.0, "MMMMMMMMM"),
    ]
    for t in test_cases:
