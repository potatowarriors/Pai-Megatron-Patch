...
def main():
    ...
    jp=[]; meta=[]
    for i,(q,r,item) in enumerate(zip(Q,R,ans)):
        a1, a2 = item[0], item[1]
    ...
    flags=[c for x in ans for c in (x[2] if len(x)>2 else [])]
    ...
    (outd/"results_logickor.json").write_text(json.dumps(
        {"results":{"logickor":{"score,none":metrics["overall"]/10.0,
                    ...
                    "think_closed,none":metrics["think_closed"]}}, "logickor_detail":metrics}, indent=2, ensure_ascii=False))
    ...
    print(f"[logickor] overall={metrics['overall']:.2f}/10  single={metrics['single_turn']:.2f}  "
          f"multi={metrics['multi_turn']:.2f}  think_closed={metrics['think_closed']*100:.1f}%", flush=True)
