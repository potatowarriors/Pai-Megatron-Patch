#!/bin/bash
#
# Alpha 프로젝트 Cooldown 학습 스크립트
# 기존 체크포인트에서 새로운 LR 스케줄로 resume하여 학습
#
# train.sh와의 차이점:
#   - Iteration/LR을 YAML에서 직접 읽음 (token→iters 변환 없음)
#   - Resume 설정 (--load, --no-load-optim) 지원
#   - WSD scheduler 관련 인자 없음 (cosine 사용)
#   - Output dir에 "cooldown" 표시
#
# 사용법:
#   bash train_cooldown.sh [model_config] [training_config] [infra_config] [data_config]
#
# 예시:
#   bash train_cooldown.sh baseline_48L stage1_cooldown h100x8 kormo_code
...
TRAINING_CONFIG=${2:-"stage1_cooldown"}
INFRA_CONFIG=${3:-"h100x8"}
DATA_CONFIG=${4:-"kormo_50pct"}

echo "=============================================="
echo "Alpha 프로젝트 Cooldown 학습 설정"
echo "=============================================="
...
#==============================================================================
# 학습 설정 로드 (cooldown YAML에서 직접 iteration 읽기)
#==============================================================================
...
if [ -z "$TRAIN_ITERS" ] || [ "$TRAIN_ITERS" = "" ]; then
    ...
    echo "   Cooldown config에는 train_iters, lr_warmup_iters, lr_decay_iters가 필요합니다."
    exit 1
fi

echo "📊 Cooldown iterations: train=${TRAIN_ITERS}, warmup=${LR_WARMUP_ITERS}, decay=${LR_DECAY_ITERS}"
...
if [ "$RESUME_ENABLED" != "true" ] && [ "$RESUME_ENABLED" != "True" ]; then
    ...
    echo "   Cooldown 스크립트는 resume 설정이 필수입니다."
    exit 1
fi
...
if [ "$WANDB_ENABLED" = "True" ] || [ "$WANDB_ENABLED" = "true" ]; then
    ...
    # Experiment name: cooldown 표시
    WANDB_EXP_NAME="${MODEL_CONFIG}_${DATA_CONFIG}_cooldown_${TIMESTAMP}"
...
#==============================================================================
# 출력 디렉토리 설정 (cooldown 표시)
#==============================================================================

OUTPUT_DIR=${CURRENT_DIR}/outputs/alpha_${MODEL_CONFIG}_cooldown_${TIMESTAMP}
...
echo "=============================================="
echo "🚀 Alpha 프로젝트 Cooldown 학습 시작!"
echo "=============================================="
echo ""
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    echo ""
    echo "=============================================="
    echo "❌ Cooldown 학습 실패 (Exit Code: ${EXIT_CODE})"
    echo "=============================================="
