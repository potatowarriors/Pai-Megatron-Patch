...
| 날짜 | 증상 | 원인 → 대응 |
|---|---|---|
| 09-14 | 에이전틱 fleet 가 도구호출 턴마다 `</think>` 소실 — SWE·TB 전 계열 | vLLM 0.25.1 qwen3_xml 파서가 reasoning 파서 없이 THINK_END 를 소비(보존 0/74,833턴). 템플릿이 이력을 `<think></think>`+추론문으로 재렌더 → 구조 왜곡. 수정은 reasoning 파서 + 추론 복원 경로(τ³ `tau_proxy.py` 방식) |
| 09-14 | τ³-bench 프록시 복원 miss 57% — 도구 호출 턴 content 에 think 가 `</think>` 없이 붙어 옴 | vLLM 0.25.1 parser engine 이 tool 파서만 켜면 THINK_END 를 삼킴 → τ fleet 는 `REASONING_PARSER=nemotron_v3` 필수(게이트 T1b). **SWE/TB-2 fleet 도 같은 content** — 조치는 결정 대기. 복원 ON 0/5 vs OFF 3/5 도 결정 대기. `KNOWN_ISSUES` 09-14 |
...
| 09-10 | 터미널 데이터를 3일 합성한 뒤 NVIDIA 공개 코퍼스(366k)를 발견 — 컬렉션 밖 단독 릴리스라 조사가 놓침. 원본은 파싱 실패 턴(think 만) 64%·null 말미 8.6%, 우리 필터는 21% 오탈락 | 조사는 조직 데이터셋 목록 전체 + arXiv 데이터 절까지(`rules/sft-data.md`). 파싱 실패 교환은 splice, 필터는 원천별 재보정, 선별 기준은 문서로 확인한 뒤 서술. `KNOWN_ISSUES` 09-10 |
...
| 09-04 | sub1 학습 스모크가 첫 스텝 전 rank `munmap_chunk(): invalid pointer`(TE cuDNN norm) — phase-1 데이터로도 동일 | compat libcuda 595 + JIT(ptxjitcompiler·nvvm) 570 혼합(09-07 진단). libcuda 는 `libcudahook` 이 강제하지만 JIT 는 LD_LIBRARY_PATH 를 따름 → **jit595 링크 디렉터리 우회로 sub1 학습 가능**(09-09 phase-3 2-iter PASS, `scripts/sub1_jit595_smoke.sh`, sudo 불필요). 영구 수정(symlink)은 root. `KNOWN_ISSUES` 09-04 |
