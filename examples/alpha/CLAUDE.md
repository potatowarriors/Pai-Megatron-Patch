...
| 날짜 | 증상 | 원인 → 대응 |
|---|---|---|
| 09-14 | 에이전틱 fleet 가 `</think>` 소실 — SWE·TB-2 전 계열, **도구 미선언 요청도** | TOOLS=1 서버 플래그만으로 vLLM 0.25.1 파서 엔진이 THINK_END 소비(SWE 보존 0/74,833턴, 무도구 6/6). 템플릿이 이력을 `<think></think>`+추론문으로 재렌더 → 구조 왜곡. 수정은 reasoning 파서 + 추론 복원 경로(τ³ `tau_proxy.py` 방식) |
