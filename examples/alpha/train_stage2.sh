#!/bin/bash
#
# Alpha 프로젝트 Stage 2 학습 스크립트
# Cooldown 체크포인트에서 WSD 스케줄로 resume하여 학습
#
# train_cooldown.sh 대비 변경사항:
#   - WSD scheduler 지원 (lr_wsd_decay_style, lr_wsd_decay_iters)
#   - save_optim YAML 기반 조건부 (하드코딩 → configurable)
#   - All-to-All 기본 인프라 (h100x8) — DeepEP 대비 ~7% 빠름 (단일 노드)
#   - 텍스트/경로: cooldown → stage2
#
# 사용법:
#   bash train_stage2.sh [model_config] [training_config] [infra_config] [data_config]
#
# 예시:
#   bash train_stage2.sh baseline_48L stage2 h100x8 stage2
