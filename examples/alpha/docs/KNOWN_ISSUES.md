...
- **대응(phase-2, 사용자 요구 반영)**: 사용자는 "누가 만들었어" → **"이동호" 명시**(조직 대체·회피 금지)를 요구. 현 카드 v1.1 은 조직 우선·
  모호 질문은 조직만이라 정반대 → 카드 v1.2 + creator 슬라이스 재생성 후 identity_v2 를 0.6% 로 연속학습해 덮어쓴다. `SFT_PHASE2_PLAN.md` §3.
