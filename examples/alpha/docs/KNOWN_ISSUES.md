...
`MIN_CACHE_GB`(기본 20) 미만이면 vllm 실행 전에 명확한 메시지로 종료한다. 기존 캐시 16G 는 지우지 않고 옮겼다
(HOME 100% → 67%). uv·pip·HF 캐시 30G 는 다른 작업 소유라 남겼다 — HOME 을 쓰는 다른 도구는 여전히 이 볼륨에
기대므로 재충전 여지가 있다.

**교훈**: 디스크 여유는 **쓰는 경로의 파일시스템**에서 잰다. 게이트 A3 는 컨테이너 호스트만 보고, sub1 서빙 노드의
캐시 볼륨을 본 검사는 없었다.
