...
**추가 (12:18~12:40, compat 차등 · 원인 확정)**: 사용자 질문 "compat 문제 아닌가" → 두 노드 차이 조사: 커널 드라이버 main1 **535.261.03** / sub1 535.230.02, compat·훅 동일.
compat 만 575.51.03 으로 되돌려(사용자 sudo, 프로세스 내 `cuDriverGetVersion` 12090 확인, jit595 제외) 같은 게이트 재실행 → **12:20:24 첫 iteration 에서 다시 정지**(8장 SM 100%·mem 0%·113~123 W·스로틀 0). → **compat 배제**(570·575·595 전부에서 재현).
정지 프로세스를 kill 하자 `nvidia-smi` 가 `Failed to fill in device global IDs` 로 abort, **GPU 7(`0000:db:00.0`)만 VBIOS `??.??.??.??.??`**, 나머지 7장 판독 정상, GPU 0 CUDA 컨텍스트도 core dump — **09-15 종착 상태 그대로**.
`nvidia-smi topo -m`: GPU 0~6 상호 NV18(18 링크) 정상, GPU 7 행은 조회 실패. NVLink 대역폭·오류 카운터 테스트는 노드 전체 CUDA 불가로 실행 불가(관리자 리셋 필요).
**판정**: GPU 7 보드 결함 **미해결 — 3회째 동일 서명**(09-15 03:47 · 09-15 17:03 · 09-18). 이번 "조치"는 리셋 직후 단독 GEMM 을 통과시킬 뿐, EP8 실부하에서 재발한다. 보드 교체(RMA) 필요.
...
**재실행 실측(17:58, 58 트라이얼)**: 정본 miss_rate **0.39%**(옛 캐시 경로 식이면 28.4% → 또 무효였을 것), distinct 턴 기준 miss 46 / 4,222 (1.1%). miss 표본은 전부 `content=""`·`reasoning_content` 없음·키 `['content','role']` — **사고가 안 닫힌 턴의 빈 assistant 메시지**(하니스가 빈 답에는 필드를 안 실음)라 복원 누수가 아니라 모델 행동(미종결 사고 69턴)이다. 규칙 통과, 무효 아님.
...
**추가 (13:19, 워치독 오탐)**: 첫 판 워치독이 `/metrics` 60 s 불통만으로 :8001(GPU 1)을 죽였다. 그때 GPU 1 은 428 W·99% 로 생성 중이었고,
API 서버 프로세스의 엔진 통계 로그만 13:17:50 부터 약 90 s 끊겨 있었다 — vLLM 은 채팅 요청의 템플릿 렌더·토크나이즈를 API 서버
이벤트 루프에서 동기로 하므로, W=96 에서 20만 토큰급 프롬프트가 몰리면 루프가 수십 초 막힌다(EngineCore 는 계속 돈다). 다른 백엔드는
100분간 30 s 초과 공백이 1건(8004, 34 s)뿐이라 드물지만 있다. 수정: 사망 판정을 "프로세스 소멸, 또는 `/metrics`·`/v1/models` 둘 다
180 s 불통" 으로, 프로브 타임아웃 20 s. 정지 판정(running>0 인데 생성 카운터 120 s 정지)은 그대로 — :8003 형 EngineCore 행은 API 서버가
살아 있어 `/metrics` 로 잡힌다. 후속 레버 후보: vLLM `--api-server-count N`(API 서버 다중 프로세스, 토크나이즈 병렬화) — 미검증.
...
**조치**: `judge_a4`(parsed / xml_unparsed / no_call) 순수 함수 + 모드별 최대 8회 재표본. XML 미파싱만 즉시 FAIL, OFF 가 전부 미호출이면 평가 조건(ON)으로 재시도해 파서를 확인하고 OFF 미호출은 ⚠️ 경고로 메시지에 남긴다. 양 모드 전부 미호출이면 통과로 쓰지 않되 "파서 문제 아님" 을 명시. `tests/test_check_agentic_gates.py` A4 7건(26 passed). 3차 기동 09:52. 교훈: **게이트 판정은 "무엇을 막는가" 의 관측과 미관측을 나눠야 한다** — 미관측을 실패로 세면 15시간짜리 단계가 조용히 빠진다.
...
lb_proxy 만 살아 있었다. 스위트는 서버가 죽은 뒤에도 준비 대기(20초 × 60회)를 돌아 **원인이 20분 뒤에야 드러나는 구조**다.

**원인**: 서버 로그(`tools/fleet_logs/serve_800x.log`) 8개 모두 `profile_run` 의 컴파일 결과 저장에서
`OSError: [Errno 28] No space left on device`. vLLM 컴파일 캐시 기본 위치가 `~/.cache/vllm` 인데 sub1 의 HOME
(`/home/work`)은 **49GB 루프 볼륨**(`/dev/loop3`)이다. NFS(`/home/work/vidsearch`, 19T 여유)·오버레이 루트(475G 여유)와
다른 파일시스템이라 `df` 를 보는 위치에 따라 여유가 전혀 달라 보인다.

| `~/.cache` 구성 (47G) | 크기 | 비고 |
|---|---:|---|
| vllm/torch_compile_cache | 16G | 서빙 설정 해시마다 쌓임(최근 7일 3.1G). 8대 동시 컴파일이 마지막 여유를 채움 |
| uv | 16G | 패키지 캐시 |
| huggingface/xet | 9.3G | 다운로드 캐시 |
| pip | 5.3G | |

**수정**: `serve_alpha.sh` 가 `VLLM_CACHE_ROOT` 기본값을 `/tmp/vllm_cache`(로컬 오버레이)로 두고, 여유가
`MIN_CACHE_GB`(기본 20) 미만이면 vllm 실행 전에 명확한 메시지로 종료한다. HOME 은 기존 캐시 16G 를 비워
100% → 67%. uv·pip·HF 캐시 30G 는 다른 작업 소유라 남겼다 — HOME 을 쓰는 다른 도구는 여전히 이 볼륨에 기대므로
재충전 여지가 있다.

**2차 실패 — 캐시를 "옮긴" 것이 틀렸다 (09-15 03:41 KST)**. 처음에는 기존 캐시 16G 를 지우지 않고 `/tmp/vllm_cache`
로 **이동**했다(되돌릴 수 있다고 봤다). T1 fleet 는 떴지만 T1·T3 뒤 에이전틱 fleet(262144 · tools · nemotron_v3)
8대가 1분 만에 전부 죽었다: `FileNotFoundError: /home/work/.cache/vllm/torch_compile_cache/bed98bab09/…/
artifact_compile_range_1_8192_subgraph_0`. **vLLM 컴파일 캐시는 원래 위치의 절대경로를 품고 있어 옮길 수 없다.**
해시가 같은 항목(오늘 스모크가 만든 에이전틱 설정)을 새 위치에서 찾은 뒤 옛 경로의 파일을 열다 실패했다.
스위트는 `fleet_up … || exit 1` 로 끝나 에이전틱·T2 가 돌지 않았다. 옮긴 캐시 42항목을 전부 지우고 에이전틱·T2 만
재기동했다(07:36 KST). **컴파일 캐시는 옮기지 말고 비운다** — 재컴파일 비용은 fleet 기동당 수 분이다.

**교훈**: 디스크 여유는 **쓰는 경로의 파일시스템**에서 잰다. 캐시는 위치 독립이라고 가정하지 않는다. 게이트 A3 는 컨테이너 호스트만 보고, sub1 서빙 노드의
캐시 볼륨을 본 검사는 없었다.
