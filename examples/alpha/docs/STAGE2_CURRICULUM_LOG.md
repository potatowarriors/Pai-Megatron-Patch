...
| 08-18 | 거울상 시소 소멸 실측 확정 (318 iter): 창 delta 상관 −0.958→+0.215, gap 진폭 0.064→0.005 (13×) |
| **08-22** | **★ stage2 완주** — 최종 iter 26,832, 양 노드 저장 완료. 최종 train loss 1.1453/1.1412, valid(p3 set) 26,000 1.1816 → 26,832 **1.1658**(양 노드 4자리 일치), LR 7.5e-6(min 도달). bias-sync 전 구간 무결(재시작 후 94/94 True), **최종 ckpt bias 노드 간 비트일치** — 배포 모델의 `e_score_correction_bias`가 결합-분포 균형으로 확정. 참고: 최종 ckpt에 diloco_outer 미동봉(종료 저장 경로가 save 훅 우회 — 학습 종료라 무해). 다음: evaluate.sh (MG→HF 변환·검증·벤치마크) |
