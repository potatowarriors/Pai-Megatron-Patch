...
_마지막 갱신: 2026-09-01 17:10 (SFT phase-2 G-P2 생성 개시 — 외부 교사 엔드포인트, 이전: 17:00 G-P1)_
...
| **SFT phase-2 (준비 중)** | **연속형 550 iters 확정 + 결정 4건 확정 2026-09-01(사용자)**: 이주성 50:50 · 존댓말 · 조직·팀 → 이름 한 문장(09-01 정정) · 550. G-P0 완료: F1 평문화(테스트 38/38) · `gen_phase2_blend.py`(리플레이 83.8%/opencode 15.5%/identity 0.6%) · preset `sft_128k_full_p2.yaml` · **identity 카드 1.2**(제작자 명시 — "…CJ주식회사 AI/DT추진실 영상콘텐츠담당 이동호입니다"·mention mix, 규칙 9 단위검증 8/8, 시드 2,000행). **G-P1 완료(16:50)**: opencode_fixed 재변환 trainable 불변·real −3.3%·`verify_sft_bins` 25/25 PASS·렌더 육안 봉투 흔적 없음. **G-P2 진행 중(08:02~)**: 교사 = 외부 `gemma4.withai.cj.net:10206`(사용자 제공) → 블로커 해소. 파일럿 ko 60행 형식 44/44·게이트 41/44 | ① 생성 완료 → export → v2 슬라이스 교체 → ×12 → 변환·verify ② F3 토큰 여부 ③ G-P4 블렌드 확정 → G-P5 스모크(phase-1 종료 ~09-06) | `SFT_PHASE2_PLAN.md` §9 |
