...
_마지막 갱신: 2026-08-30 18:10 (T1 프론티어 규약 재작성·게이트 G1~G3 통과·스모크 검증 완료. SFT 본 런 iter 527/2448 정상)_
...
| **SFT 벤치마크 (sub1)** | **T1 재구축 완료·게이트 통과 (2026-08-30).** 프론티어 규약 조사 후 T1 5종을 커스텀 재작성: `eval_sft/tasks/{mmlu_pro,gpqa_diamond,ifeval,aime25,hmmt_feb_2025}_aa.yaml` + `aa_utils.py`. **온도 1.0 확정(사용자)** — Nemotron 3 Ultra 동일. 0-shot CoT·AA 표준 프롬프트·8단 폴백 추출·avg@k 실제 작동·`skip_special_tokens: false`. 발견·수정한 별개 결함 3건(GPQA strict 정규식이 채팅 모델에 원리적 0점, MMLU-Pro 5-shot, avg@16이 실질 avg@1) 은 `KNOWN_ISSUES.md` 2026-08-30. **게이트 G1~G3 실측 통과**(`check_gates.py`): 쉬운 질문 369토큰 finish=stop(이전엔 32K 폭주), `</think>` 관측 OK. G1은 `run_convert.sh` 에 내장돼 변환마다 자동 실행. 스모크(limit=2, iter300) 전 태스크 '유효' 판정 — 추출 0% 실패, GPQA 답변부 718자/전체 14,460자로 사고 분리 확인. **미해결**: 어려운 문제에서 `</think>` 미마감(finish=length) — 모델 미성숙(300/2448=12%), 설정 문제 아님. T2 RULER 재설계·T3·에이전틱은 미착수 | ① 본 측정(iter600+ 권장) ② RULER 추론 OFF 재설계 ③ T3·SWE·Terminal 를 같은 게이트 체계로 | `SFT_BENCHMARKS.md` §3.4·§3.6·§7, `KNOWN_ISSUES.md` |
