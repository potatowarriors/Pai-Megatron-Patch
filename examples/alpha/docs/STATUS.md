...
| **SFT 벤치마크 (sub1)** | 인프라 구축 완료 (2026-08-29): ① 판정 정본 `SFT_BENCHMARKS.md`(DSV4 26종→채택 8·보강 5·차단 13, 4티어). ② vLLM 서빙 = vllm 0.25.1+플러그인, **서빙 패리티 PASS**(cos≥0.99998), CUDA13 compat 570→595 교체(사용자 승인, 회귀 없음). ③ 멀티GPU = vLLM DP munmap 크래시 → **단일서버 7개+라운드로빈 프록시**(`serve_fleet.sh`/`lb_proxy.py`, GPU1~7; GPU0은 누수로 제외). ④ T1 러너(`run_tier1.sh`, lm_eval chat-completions) 스모크 PASS(추출·채점 정상). ⑤ SWE-bench 5.0.2 gpu06 DinD 검증(gold RESOLVED). judge=gemini-3.7-flash(검증). **진행: LC-B iter320 베이스라인 전량 T1 실행 중** | 베이스라인 완료 → 첫 SFT ckpt(iter300+) 측정 → T2(RULER/MRCR)·T3(judge)·SWE 러너 | `SFT_BENCHMARKS.md`, `EVAL_DOCKER_NODE.md` |
