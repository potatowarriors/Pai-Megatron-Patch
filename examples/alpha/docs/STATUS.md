...
_마지막 갱신: 2026-08-30 16:45 (벤치 트랙 전면 정리 — 무효 수치 1.4GB 삭제, 원인 규명 완료. SFT 본 런은 iter 525/2448 정상 진행 중)_
...
| **SFT 벤치마크 (sub1)** | **전면 재설계 필요 (2026-08-30).** 기존 수치 전량 무효·삭제 — HF 변환 ckpt에 `generation_config.json`이 없어 eos가 `<|endoftext|>`(0)뿐, SFT 턴 종료 `<|im_end|>`(3)에서 안 멈춤 + `</think>` special=True로 출력에서 삭제 → 모델이 아니라 서빙 설정을 측정했다. 경위·증거·재발방지 5조건은 `KNOWN_ISSUES.md` 2026-08-30 항목. **프로브로 확정된 별건**: 설정을 고쳐도 iter300 모델은 어려운 문제에서 `</think>`를 닫지 못한다(finish=length, reasoning 48k자, content 0자) — 300/2448=12% 학습의 미성숙이며 설정 수정만으로 점수가 나오지 않는다. RULER 128토큰 출력예산도 reasoning 모델엔 무효(LC-B 자체 NIAH는 200/200). **살아있는 인프라**(재사용 가능): serve venv+vllm 0.25.1 플러그인, 서빙 패리티 cos≥0.99998(`parity_iter320b.log`), ckpt 변환 검증 14181/14181(`validate_iter300.log`), lm_eval 0.4.12, T1~T4 러너·태스크 yaml, SWE-bench 5.0.2·Terminal-Bench 0.2.18 gpu06 설치(gold/oracle 통과), gemini judge. sub1 GPU 8장 전부 해제 | ① 변환기에 `generation_config.json` 생성+eos 정합 게이트 ② 서빙 1건 스모크 게이트 ③ RULER 예산 재설계 ④ 게이트 통과 후 재측정 (모델 성숙도 감안해 iter600+ 권장) | `SFT_BENCHMARKS.md`, `EVAL_DOCKER_NODE.md`, `KNOWN_ISSUES.md` |
