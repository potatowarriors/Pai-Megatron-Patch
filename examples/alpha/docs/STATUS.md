...
| **SFT phase-2 (준비 중)** | **연속형 550 iters 확정 + 결정 4건 확정 2026-09-01(사용자)**: 이주성 50:50 · 존댓말 · 조직명 후행 · 550. G-P0 완료: F1 평문화(테스트 38/38) · `gen_phase2_blend.py`(리플레이 83.8%/opencode 15.5%/identity 0.6%) · preset `sft_128k_full_p2.yaml` · **identity 카드 1.2**(제작자 개인 우선·조직 후행·mention mix, 규칙 9 단위검증 8/8, 시드 2,000행). **블로커**: identity_v2 생성용 교사 vLLM GPU 창구 없음(main1 phase-1·sub1 벤치 fleet 점유) | ① G-P1 opencode 재변환(CPU, sub1) ② GPU 창구 나면 G-P2 identity_v2 생성(≈1h) ③ F3 토큰 여부 ④ G-P4 블렌드 확정 → G-P5 스모크 | `SFT_PHASE2_PLAN.md` §9 |
