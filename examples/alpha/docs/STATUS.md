...
_마지막 갱신: 2026-09-01 17:00 (SFT phase-2 G-P1 완료·카드 1.2 형식 정정, 이전: 09:30 G-P0)_
...
| **SFT phase-2 (준비 중)** | **연속형 550 iters 확정 + 결정 4건 확정 2026-09-01(사용자)**: 이주성 50:50 · 존댓말 · 조직·팀 → 이름 한 문장(09-01 정정) · 550. G-P0 완료: F1 평문화(테스트 38/38) · `gen_phase2_blend.py`(리플레이 83.8%/opencode 15.5%/identity 0.6%) · preset `sft_128k_full_p2.yaml` · **identity 카드 1.2**(제작자 명시 — "…CJ주식회사 AI/DT추진실 영상콘텐츠담당 이동호입니다"·mention mix, 규칙 9 단위검증 8/8, 시드 2,000행). **G-P1 완료(16:50)**: opencode_fixed 재변환 trainable 불변·real −3.3%·`verify_sft_bins` 25/25 PASS·렌더 육안 봉투 흔적 없음. **블로커**: identity_v2 생성용 교사 vLLM GPU 창구 없음(main1 phase-1·sub1 벤치 fleet 점유) | ① GPU 창구 나면 G-P2 identity_v2 생성(≈1h) → 변환 ② F3 토큰 여부 ③ G-P4 블렌드 확정 → G-P5 스모크(phase-1 종료 ~09-06) | `SFT_PHASE2_PLAN.md` §9 |
