...
_마지막 갱신: 2026-09-01 17:40 (identity_v2·swap 블렌드 완료, 오케스트레이터 가동, 이전: 17:35 전환)_
...
| **SFT phase-1 iter 1200 데이터 교체 (대기 중)** | **사용자 결정 2026-09-01: phase-2 연속형 대신 iter 1200 에서 수정 블렌드로 재개**(`SFT_PHASE2_PLAN.md` §10). **준비 완료**: opencode_fixed(trainable 불변·verify PASS) · **identity_v2**(카드 1.2 creator 1,145행 교체 → 7,220행·111 bins·verify PASS·렌더 신형식 3,456/v1 0) · 블렌드 `sft_128k_mixed_blend_swap.yaml`(**phase-1 가중치 비트 동일·경로 2개만 치환 → 순열 보존, 경계 중복 0**; identity_v2 0.43%·opencode_fixed 잔여 0.16ep, 누적 = 설계값) · preset `sft_128k_full_swap.yaml` · 프로브 하니스. **오케스트레이터 가동(08:35)**: iter_0001200 저장(≈20:14) 확인 → sanity → 중단 → 재개 → 1201~1205 loss 기록. chat 복원(F3) **종결 — 회수 불가**(lmsys 공개판 부재·WildChat-Full 계정 미승인 403), 88.2% 정본 | ① 교체 후 1201~1205 loss ±0.05·LR 연속 확인 ② iter1200 HF 변환 → 정체성 프로브 기준선 ③ 2448 종료 후 프로브·T1·에이전틱 | `SFT_PHASE2_PLAN.md` §10 |
