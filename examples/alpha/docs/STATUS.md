...
_마지막 갱신: 2026-09-01 08:30 (SFT phase-2 연속형 확정 + 제작자 귀속 정책 변경 반영, 이전: 07:00 phase-2 계획 착수)_
...
| **SFT phase-2 (계획)** | **연속형 확정 2026-09-01(사용자).** phase-1 최종 ckpt 에서 500~600 iters(550 기준 2.1일) 연속 학습. 결함 3건 심각성 = ① 중간 · ② 낮음~중간 · ③ 낮음(재실행 강제 없음, `KNOWN_ISSUES.md` 09-01). 설계 = phase-1 블렌드 리플레이 ≈80% + opencode_fixed 부스트(총 0.25ep) + chat 복원분(조건부) + **identity_v2 0.6%**. **제작자 귀속 정책 변경(사용자)**: "누가 만들었어" → "이동호" 명시, 조직 대체·회피 금지 → 카드 v1.2 + creator 슬라이스 재생성 선결. LR 1.0e-5→1.5e-6, `finetune: true`, save/eval 100 | ① 사용자 결정 4건(§7: 이주성 처리·문체·조직 후행·예산) ② G-P0~P4 (sub1 교사 vLLM + CPU) ③ G-P6 phase-1 기준선 | `SFT_PHASE2_PLAN.md` |
