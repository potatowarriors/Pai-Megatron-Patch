...
_마지막 갱신: 2026-09-01 18:45 (iter 900 즉시 교체 실행, 이전: 17:40 준비 완료)_
...
| **SFT phase-1 iter 900 데이터 교체 (실행 중)** | **사용자 결정 2026-09-01 09:45: 1200 대기 대신 iter_0000900 에서 즉시 교체**(`SFT_PHASE2_PLAN.md` §10). 되돌린 구간 901~1083(≈16h), 잔여 1,548 iters. 블렌드 = phase-1 가중치 비트 동일·경로 2개 치환(opencode_fixed, identity_v2) → 순열 보존·경계 중복 0(§10.4). identity_v2 0.43%(≈121회)·opencode 누적 0.31ep = 원설계. 오케스트레이터(swap_at_900) 실행: sanity → 895~900 loss 기록 → phase-1 SIGTERM → 재개 `sft_128k_full_swap`×`sft_128k_mixed_blend_swap` → 901~905 감시 | ① 901~905 train loss ±0.05·LR 연속·캐시 24셋 재사용 확인 → 계보 기록 ② hfmodel_0000900 으로 정체성 프로브 기준선(fleet 여유 시) ③ 2448 종료(~09-07) 후 프로브·T1·에이전틱 | `SFT_PHASE2_PLAN.md` §10 |
