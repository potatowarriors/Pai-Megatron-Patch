...
|---|---|---|---|
| **LC-B** 128K@CP8+offload | **▶ 재개2 가동 (08-26 22:38)**: run `outputs/alpha_baseline_48L_lc_b_resume_20260826_223836` — ckpt 200(optim 승계)에서 속행, 잔여 120 iters → **완주 ~08-27 06시**. 재개1(172926, iter151~)은 iter 222에서 rank6 TE fused-attn `CUDNN_STATUS_EXECUTION_FAILED_CUDA_DRIVER`로 사망 — ECC 0·XID 무흔적·70 iter 정상 후 발생이라 **일과성 판정**, 재발 시(특히 rank6) 하드웨어 의심 승격. 중간 검증 5종 전부 PASS(iter~165 의도 중단, `outputs/lc_b_midrun_eval/`): ① CP8 backward 등가(grad 8랭크 일치) ② 위치별 NLL 격차 위치-단조 +0.089@120k·≤16k 동률 ③ NIAH LC-B 200/200 ④ GBS 노이즈 σ비 2.07=√4 산술 일치 ⑤ **표준 벤치 10종 평균 Δ−0.10pp — 파괴적 망각 없음**(worst HumanEval −1.83pp<1σ; MBPP 결과 누락만 미해결). 판정: loss 평탄은 측정 노이즈, 학습 정상. 벤치는 lm_eval 0.4.12 격리 설치(`/home/work/vidsearch/tools/lmeval0412`, main1 0.4.0과 별도) — LC-A 기준선과 동일 버전. **SFT 자동 시작 금지 — 완주 정본 신호는 ckpt latest==320** (현재 150). run1: `alpha_baseline_48L_lc_b_20260826_033011`(iter 150까지). | 완주 → 평가. 병행: RULER@128K 하니스 구축. 미착수(의도): EN 다문서 합성 | `LC_DATASETS.md`, `MUON_OFFLOAD_BACKPORT.md`, `configs/training/lc_b.yaml`·`lc_b_resume.yaml` 헤더 |
