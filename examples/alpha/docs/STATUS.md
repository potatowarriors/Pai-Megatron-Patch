...
_마지막 갱신: 2026-09-01 17:35 (iter 1200 데이터 교체로 전환·준비 중, 이전: 17:10 G-P2 개시)_
...
| **SFT phase-1 iter 1200 데이터 교체 (준비 중)** | **사용자 결정 2026-09-01: phase-2 연속형 대신 iter 1200 에서 수정 블렌드로 재개** (추가 GPU 0, identity 덮어쓰기 신호 ≈3배, `SFT_PHASE2_PLAN.md` §10). 준비 완료: opencode_fixed(trainable 불변·verify PASS) · preset `sft_128k_full_swap.yaml`(finetune 제거, load=자기 ckpt) · 카드 1.2(형식 "…CJ주식회사 AI/DT추진실 영상콘텍츠담당 이동호입니다", 파일럿 44/44) · 프로브 하니스. 진행 중: identity_v2 생성(외부 gemma-4-12B-it, 08:02~) → 체인(export·v2 교체·×12·변환·verify) · chat 복원 재시도(WildChat-1M-Full, `.env` 토큰, 08:23~). **iter_0001200 ≈ 20:14** 저장 후 오케스트레이션(sanity 실패 시 학습 유지)으로 교체 | ① identity_v2 bins·verify ② 블렌드 swap 산출·커밋 ③ 20:14 교체 → 1201~1205 loss ±0.05 확인 ④ 2448 종료 후 프로브·T1·에이전틱 | `SFT_PHASE2_PLAN.md` §10 |
