...
|---|---|
| **서빙** | |
| `serve_alpha.sh` | 단일 vLLM 서버. `TOOLS=1` 이면 `--enable-auto-tool-choice --tool-call-parser ${TOOL_PARSER:-qwen3_xml}`. `PREFIX_CACHE=1`(기본, run_suite) → `--enable-prefix-caching --mamba-cache-mode align`, `MAMBA_BLOCK`·`MAX_BATCHED_TOKENS` 선택. `GPU_MEM_UTIL` 기본 **0.95**(2026-09-17 사용자 결정, 다음 기동부터; 이전 0.90) |
...
| **final_resume_0915 iter600 (SFT 최종 런)** | ✅ | ✅ | ✅ | ✅ 1.8% | ✅ TB-2 0.1% · τ³ airline 30.5 / retail 무효 | **main1 GPU 0~6**(GPU 7 결함 제외, 변환 EP=6·fleet 7대) 09-16 23:18 → 09-17 06:00. AIME·HMMT 무효(사고마감 16%·12%, 전 ckpt 동일). 에이전틱은 **A4 1회 표본 오판**(모델이 도구 대신 되물음 → 파서 실패로 오독, A5 는 2/2 PASS)으로 건너뜀 → A4 를 다중 표본으로 고친 2차(09:47)도 thinking OFF 8/8 미호출("bash 도구를 쓸 수 없다")로 FAIL, A5 는 4/4 PASS → A4 에 평가 조건(ON) 폴백 추가 후 **3차 09:52 `run_suite.sh … agentic` 재기동**(`KNOWN_ISSUES` 09-17). SWE 는 11:00 정지 후 prefix caching·세션 고정·W=96 으로 4차 재개(64건 승계) → **9/500 = 1.8%**(Submitted 27%·스텝 한도 초과 52%). TB-2 는 101 트라이얼에서 miss_rate 32.5% 무효 오판(분모 버그, `KNOWN_ISSUES` 09-17)으로 중단 → 수정 후 **재실행 완료 09-18 03:59: 1/701 = 0.1%**(AgentTimeoutError 79%, 119/712 트라이얼 컨텍스트 262K 초과, miss_rate 1.0%; TB-2 첫 완주 기준선). **τ³ 완료 05:30**: airline pass^1 30.5(pass^4 12.0, harness_fail 0) 유효 · retail 무효(NL-assertion 판정 LLM `gpt-4.1` 하드코딩 → 404, `KNOWN_ISSUES` 09-18). 관찰: RULER single_2 131K **0.25**(phase-1 iter600 0.90) — 실패 43건 중 37건이 `0, 1`·`0.5, 1.0, …` 퇴행 출력(Reasoning-Off 조건, single_1 은 100%). LogicKor 31.2(phase-1 iter600 40.2). MMLU-Pro 49.5·GPQA 35.4·IFEval 61.1 은 phase-1 iter600(48.6/32.2/55.6) 우위 |
