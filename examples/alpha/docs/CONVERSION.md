...
bash scripts/convert_to_hf.sh \
  outputs/alpha_baseline_24L_20250117/checkpoints/iter_0010000 \
...
model = AutoModelForCausalLM.from_pretrained('hf_models/alpha_baseline_24L_iter10k')
...
python scripts/validate_conversion_config.py \
  hf_models/alpha_baseline_24L \
  --from-yaml configs/model/baseline_24L.yaml \
  --fix
```
...
Alpha 전용 converter가 새로 추가되었습니다. Qwen3-Next converter를 기반으로 Alpha baseline_24L 설정에 맞게 최적화되었습니다.
...
- **Alpha 전용 설정**: baseline_24L 하이퍼파라미터 내장
