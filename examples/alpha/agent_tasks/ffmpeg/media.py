...
def _video_src(spec):
    ...
    src = f"{pattern}=s={w}x{h}:r={fps}:d={dur}"
    chain = []
    # black: [start, end] -> frames in that window are forced to black (blackdetect tasks)
    if "black" in spec:
        a, b = spec["black"]
...
def _audio_src(a, dur):
    hz, vol = a.get("hz", 440), a.get("volume", 0.5)
    src = f"sine=f={hz}:r={a.get('sr', 48000)}:d={dur},volume={vol}"
...
def make_video(spec, path):
    ...
    subs = spec.get("subs")
    if subs:
        srt = Path(path).with_suffix(".embed.srt")
        write_srt(subs, srt)
        cmd += ["-i", str(srt)]
    cmd += ["-map", "0:v"]
    for i in range(len(audios)):
        cmd += ["-map", f"{i + 1}:a"]
    if subs:
        cmd += ["-map", f"{len(audios) + 1}:s", "-c:s", "srt" if str(path).endswith(".mkv") else "mov_text"]
