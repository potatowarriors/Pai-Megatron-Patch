...
#   300 iters 마다(ckpt 저장 주기, 사용자 09-14): probe_ckpt.sh (MG→HF 변환 sub1 8 GPU + identity 프로브 + 유령 호출 재생) → eval_sft/results/probe/
...
while true; do
  ...
  for d in $(ls -d "$RUN_DIR"/checkpoints/iter_* 2>/dev/null | sort); do
    ...
    if [ $((it % ${PROBE_EVERY:-300})) -eq 0 ] && [ ! -f "eval_sft/results/probe/$tag.done" ] && [ ! -f "eval_sft/results/probe/$tag.tried" ]; then
