...
# This script validates that the timeout parameter has been correctly added
...
COUNT=$(grep -A 7 "Build the expert model parallel group" \
   ../../backends/megatron/Megatron-LM-250908/megatron/core/parallel_state.py | \
   grep -c "timeout=timeout" || true)
if [ "$COUNT" -ge 1 ]; then
...
COUNT=$(grep -A 7 "Build the expert tensor parallel group" \
   ../../backends/megatron/Megatron-LM-250908/megatron/core/parallel_state.py | \
   grep -c "timeout=timeout" || true)
if [ "$COUNT" -ge 1 ]; then
    echo -e "${GREEN}PASS${NC}"
...
if grep -q "atexit.register(cleanup_distributed)" ../../megatron_patch/training.py; then
    ...
    echo "  WARNING: atexit handler not registered (optional)"
fi
