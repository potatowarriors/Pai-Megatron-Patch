...
def main():
    ...
    body = {"text": PROMPT, "messageId": str(uuid.uuid4()), "parentMessageId": "00000000-0000-0000-0000-000000000000",
            "conversationId": None, "isCreatedByUser": True, "sender": "User", "endpoint": endpoint,
            "endpointType": "custom", "model": model, "isTemporary": True,
            "ephemeralAgent": {"execute_code": False, "web_search": False, "mcp": []}, "timezone": "Asia/Seoul"}
    start = json.load(http(f"{LC}/api/agents/chat/{endpoint}", body, token=tok))
    sid = start.get("streamId")
    check("채팅 요청 수락", bool(sid), f"status={start.get('status')}")
    if not sid:
        return
    final = None
    t0 = time.time()
    for raw in http(f"{LC}/api/agents/chat/stream/{sid}", token=tok, stream=True, timeout=600):
