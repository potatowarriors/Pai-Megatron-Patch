...
echo "[launch_diloco] tag=$TAG presets=$MODEL/$TRAIN/$DATA H=${DILOCO_H:-30} tau=${DILOCO_TAU:-0} port_base=$PORT_BASE node1_seed=$NODE1_SEED args=$*"

ssh -o StrictHostKeyChecking=no sub1 \
  "cd $ALPHA && nohup env $ENVV DILOCO_RANK=1 bash train.sh $MODEL $TRAIN $DATA $* --seed $NODE1_SEED ${NODE1_ARGS:-} > \$HOME/run_diloco_${TAG}_node1.log 2>&1 < /dev/null & echo node1_pid=\$!"
