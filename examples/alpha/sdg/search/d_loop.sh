...
case "$1" in
  questions)
    for r in $(seq 1 60); do
      GEN_THOROUGH=0 python3 wd_question.py --chains out/d1/chains.jsonl --out out/d1/questions.jsonl --workers 48 2>&1 | grep -E "^DONE" | cut -c1-120
  ...
  d1)
    for r in $(seq 1 60); do
      AGENT_TEACHER=dsv4-flash python3 agent_run.py --questions out/d1/questions.jsonl --out out/d1/traj.jsonl --search http://localhost:8600 --workers ${D1_WORKERS:-120} --mode d1 2>&1 | grep -E "^DONE" | cut -c1-140
  ...
  d2)
    for r in $(seq 1 30); do
      ...
      AGENT_TEACHER=dsv4-flash python3 agent_run.py --questions out/d2/requests_all.jsonl --out out/d2/traj.jsonl --search http://localhost:8600 --workers ${D2_WORKERS:-60} --mode d2 --max-calls 15 --max-tokens 12000 2>&1 | grep -E "^DONE" | cut -c1-140
