...
- 원천 비율: math 162.7k(44%) vs code+swe+synthetic 203k. ~~7:3 서브샘플~~ → **비율 정책 폐기(사용자 09-10)**. **사용자 확인(09-10)**: 전량 365,705행(품질 필터 미적용 —
  논문 무필터 결론과 정합, splice 만 적용) · math 어댑터 포함 · 수집 원본 보존. bins 는 분할별 6 멤버(`ntc_v1_code/math/swe/syn_easy/syn_medium/syn_mixed`)로 구워
  블렌드에서 크기 비례 가중이면 단일 멤버와 동치이고, 분할 조정이 필요하면 재빌드 없이 가중만 바꾼다.
