# [아카이브] 터미널 에이전트 SFT 자체 합성 트랙 (2026-09-07 ~ 09-10, 폐기)

> **폐기됨 (2026-09-10, 사용자 결정)**: NVIDIA 가 `nvidia/Nemotron-Terminal-Corpus`(366k행)를 공개해 직접 합성을 중단했다. 산출물 `alpha-SFT-Terminal-v1`·bins
> `terminal_terminus2_synth` 는 삭제. 수집 원본(`/home/work/vidsearch/tools/glm53/{collect,tasks,pilot}`, 9.8 GB)은 보존. 현재 정본은 [`README.md`](README.md).
> 이 문서는 당시 결정·파이프라인·실측·사고를 그대로 보존한 기록이다. 코드(`tasks/ collect/ serve/ gates/ pilot/ convert/traj_to_terminus.py assemble_dataset.sh`)는 남아 있으나 학습 경로에서 쓰지 않는다.
>
> **폐기와 함께 무효가 된 설정** (삭제하지 않고 헤더에 폐기 표시): `configs/training/sft_128k_terminal_p3.yaml`, `configs/data/sft_128k_terminal_blend_p3.yaml`,
> `scripts/launch_p3_after_phase2.sh`, `toolkits/sft_data_preprocessing/{gen_sft_128k_terminal_blend_p3.sh, terminal_p3_replay_shares.tsv}`. 이 트랙에서 나온 **유효한 수정**은 유지:
> `--mid-level-dataset-surplus` 배관(63f79d2)·`sub1_jit595_smoke.sh`·`filter_rows.py` 보정·`dataset_stats.py`·`--keep-history-think` 변환 옵션·`swe_v3_terminus_keephist` 멤버.
