...
#   Hopper 는 FP4 indexer cache 비활성(--attention_config.use_fp4_indexer_cache False). MoE 백엔드: 0731 experts 는 MXFP4 라 Triton 이 SiLU 를 거부 → **marlin**(실측 2026-09-10). seqs 256 (P1: 128→256, 4,954 tok/s@128 기준선).
...
echo "[serve-dsv4] model=$MODEL TP8+EP kv=fp8 moe=${MOE_BACKEND:-marlin} seqs=$MAX_SEQS max_len=$MAX_LEN port=$PORT venv=$VENV"
