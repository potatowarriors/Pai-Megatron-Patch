...
### P2-B 완료 (2026-09-10 14:30 KST) — DSV4 생성·GLM 심판, `out/p1/gen_B.jsonl` 41,448행 (`out/p1/B_INSPECT.md`)

채택률 99.2%(양샘플 리젝 348), 심판 실패 9, 판정 A/B 20,007:20,200(순서 편향 없음), 자기귀속 0, 한자 ≈0, 멀티턴 579(1.4%). 내보내기 최종 게이트 리젝 0 → thinking 29,208 · no-think 12,240(29.5%).
샘플 리젝 사유(전체 ≈1.5%): 지시문 누출 426·빈 답변 222·중국어 280·퇴행 146·길이 초과 79·자기귀속 75.

| 트랙(DSV4) | 행 | 사고 tok p50 (p25/p75) | 답변 tok p50 | 사고 한글비 | 답변 한글비 |
|---|---|---|---|---|---|
| S1 현지화 | 12,332 | 568 (235/1,267) | 597 | 0.71 | 0.78 |
| S2 기사 A(문서 제공) | 9,822 | 503 (283/947) | 399 | 0.89 | 0.90 |
| S2 기사 B(영감형) | 9,727 | 616 (227/1,021) | 930 | 0.91 | 0.98 |
| S3 실사용자 | 1,163 | 429 (200/862) | 505 | 0.65 | 0.78 |
| S4 한국 맥락 | 8,404 | **1,261** (504/1,962) | 1,135 | 0.72 | 0.98 |
| (대조) NVIDIA Chat-v3 | — | 1,256 (970/1,606) | 878 | — | — |

DSV4 는 P0 대로 NVIDIA 의 0.4~0.5×(한국어 사고), S4 만 1×. GLM 측(A)이 2~2.7× 이므로 둘을 합쳐야 NVIDIA 근처 — **A:B 비율을 바꾸면 사고 길이 분포도 같이 움직인다**(재배분 결정 시 고려).
P2-A 는 진행 중(09-10 14:30 기준 ≈2.5k 행, 0.13 시드/s): GLM 하이브리드 동시성 캡·폭주 24%·S4 부적합 — 결정 대기(v2 지시문·S4 DSV4 전담), README 위 P1 절.

### P2 완료 → P3 검수 → P4 변환 (2026-09-11 18:10 KST) — `reports/P3_REPORT_P1.md`

**생성 92,924행**(시드 93,494 의 99.4%) = B(DSV4 생성·GLM 심판) 46,304 · A(GLM 생성·DSV4 심판) 42,305(직접 13,879 + GLM 2대 병행 A1 14,121·A2 14,305, 심판 보류 후 DSV4 일괄 판정) · A 리젝 구제(DSV4) 4,315.
교사 비율 GLM 45.5% / DSV4 54.5%(구제분이 DSV4 로 기움). 판정 편향 없음(GLM 측 A/B 18,506:18,785 · DSV4 측 24,237:24,393), 심판 실패 12, 자기귀속 0, 한자 ≈0, 멀티턴 1,145(1.2%).

| 교사 | 트랙 | 사고 tok p50 (p25/p75) | 답변 tok p50 | 사고 한글비 |
|---|---|---|---|---|
| GLM | S1 / S2-A / S2-B / S3 / S4 | 2,721 / 3,158 / 3,206 / 1,947 / 6,207 | 522 / 515 / 787 / 479 / 967 | 0.13~0.39 (영어) |
| DSV4 | S1 / S2-A / S2-B / S3 / S4 | 622 / 513 / 715 / 568 / 1,403 | 669 / 469 / 949 / 383 / 1,168 | 0.60~0.91 (한국어) |
| **혼합 전체** | | **1,408** (540/3,504, p90 6,507) | 742 | |
| (대조) NVIDIA Chat-v3 | | 1,256 (970/1,606) | 878 | |

- **GLM 2대 병행(09-11 01:51~16:02, 사용자 승인)**: sub1 DSV4 를 내리고 GLM(EP+DP8, jit595)을 올려 A 잔여 31,650 시드를 해시 반분 → main1 4,787 · sub1 4,918 tok/s, 합산 0.72~0.78 시드/s(2배). 심판은 `--defer-judge` 로 두 샘플 보존 후 `judge_pending.py`(DSV4 재기동, 28k행 40분). 
- GLM Max effort 폭주: S1~S3 양샘플 리젝 7~10%, S4 25~39% → 리젝 시드는 `--skip-rejected` 로 재시도 차단, DSV4 구제 루프(회수율 83~92%)로 4,315행 회수. 두 교사 모두 실패 ≈500 시드 폐기.
- **P4 변환** `sft_packed_128k_terminal_pad16/kochat_v3_{think,nothink}` (128k 레시피, `metadata.train_turns` 마지막만): thinking 65,201행 → **1,709 bins, 실 223.5M·학습 210.7M 토큰**(fill 0.998) · no-think 27,723행(29.8%) → **214 bins, 실 27.8M·학습 22.6M**. 드롭 0, `verify_sft_bins` **PASS**(300/1,709·214/214 bad 0), `render_check` clean(RENDER_CHECK.md 기록; 멀티턴 히스토리 턴은 빈 `<think></think>` 렌더·비학습).
- 결정 미반영(사용자 무응답 → 현행 유지): v2 지시문, S4 DSV4 전담, A:B 재배분.
